...
foreach (Atlas atlas in packer.Atlasses) {
	...
	foreach (Node n in atlas.Nodes) {
		if (n.Texture != null) {
			...
			texturePageItem.SourceX = (ushort)n.Bounds.X; texturePageItem.SourceY = (ushort)n.Bounds.Y; texturePageItem.SourceWidth = (ushort)n.Bounds.Width; texturePageItem.SourceHeight = (ushort)n.Bounds.Height;
			texturePageItem.TargetX = 0; texturePageItem.TargetY = 0; texturePageItem.TargetWidth = (ushort)n.Bounds.Width; texturePageItem.TargetHeight = (ushort)n.Bounds.Height;
			texturePageItem.BoundingWidth = (ushort)n.Bounds.Width; texturePageItem.BoundingHeight = (ushort)n.Bounds.Height;
			texturePageItem.TexturePage = texture;
			Data.TexturePageItems.Add(texturePageItem);
			string stripped = Path.GetFileNameWithoutExtension(n.Texture.Source);
			string spriteName = n.Texture.Directory;
			int frame = Int32.Parse(stripped);
		...
		foreach (FileInfo fi in files) {
			...
			if (img != null) {
				if (img.Width <= AtlasSize && img.Height <= AtlasSize) {
					...
					ti.Directory = fi.Directory.Name;
